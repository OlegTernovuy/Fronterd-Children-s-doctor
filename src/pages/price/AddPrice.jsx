...
export const AddPrice = ({ isAdministrator }) => {
  ...
  const getData = async () => {
    try {
      setIsPending(true);
      const responce = await fetch(`${url}/prices`);
  ...
  const deletePrice = async (MyPrice) => {
    if (
      ...
      try {
        setIsPending(true);
        const responce = await fetch(`${url}/prices/` + MyPrice._id, {
          method: "DELETE",
        });
        if (responce.status === 200) {
          getData();
  ...
  const EditPrice = async (updatePrice) => {
    try {
      setIsPending(true);
      const responce = await fetch(`${url}/prices/` + updatePrice.id, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
